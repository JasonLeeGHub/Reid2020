...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        # attention
        self.attention_layer = nn.Sequential(nn.Conv2d(2048, 3, kernel_size=1),
                                             nn.BatchNorm2d(3),
                                             nn.ReLU())
        self.batchnorm2048 = nn.BatchNorm2d(1)
        self.batchnorm4096 = nn.BatchNorm2d(4096)
        self.ReLU = nn.ReLU(inplace=True)

    def forward(self, x):
        x = self.resnet_conv(x)
        #  1*1 convolution for attention maps
        attention_map = self.attention_layer(x)
        attention_map = torch.sum(attention_map, dim=1).unsqueeze(1)
        attention_map = self.batchnorm2048(attention_map)
        feature_map = x
        for i in range(attention_map.size()[1]):
            attention_muled_i = attention_map[:, i, :, :].unsqueeze(1).repeat((1, x.size()[1], 1, 1)) * x
            feature_attention_cat = torch.cat((feature_map, attention_muled_i), dim=1)
            feature_attention_cat = self.batchnorm4096(feature_attention_cat)
            feature_attention_cat = self.ReLU(feature_attention_cat)
        return feature_attention_cat
...
class Embeder(nn.Module):
    ...
    def __init__(self, part_num, class_num):
        ...
        for i in range(part_num):
            ...
            setattr(self, name, BottleClassifier(4096, self.class_num, relu=True, dropout=False, bottle_dim=256))
            name = 'embedder' + str(i)
            setattr(self, name, nn.Linear(4096, 256))
