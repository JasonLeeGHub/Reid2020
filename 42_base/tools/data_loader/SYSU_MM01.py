...
# put dataset into root

class SYSU_MM01(BaseImageDataset):
    dataset_dir = 'SYSU-MM01'
    def __init__(self, root, reorder=True, **kwargs):
        ...
        self.rgb_train = self.rgb_samples_train
        self.rgb_query = self.rgb_samples_val
        self.rgb_gallery = self.rgb_samples_test
    ...
    def _load_samples(self):
        ...
        for cam_id in self.rgb_camera_ids:
            cam_path = os.path.join(self.dataset_dir, 'cam{}'.format(cam_id))
            _, person_ids, _ = self.os_walk(cam_path)

            for person_id in person_ids:
                person_path = os.path.join(cam_path, person_id)
                _, _, image_names = self.os_walk(person_path)
        ...
        print('Note: pids of rgb_train_set and ir_train_set are {} equal'.format(
            self._is_equal(self.rgb_samples_train, self.ir_samples_train, 1)))
        print('Note: pids of rgb_val_set and ir_val_set are {} equal'.format(
            self._is_equal(self.rgb_samples_val, self.ir_samples_val, 1)))
...
if __name__ == '__main__':
    Dataset = SYSU_MM01(root='/media/Nuke/2020ReID/SYSU-MM01/')
